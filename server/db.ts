...
export async function createApplication(data: InsertApplication): Promise<Application> {
  ...
  const app = await db.select().from(applications).where(eq(applications.id, id as number)).limit(1);
  return app[0]!;
}
...
export async function updateApplicationStatus(id: number, status: "pending" | "accepted" | "rejected"): Promise<Application> {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
